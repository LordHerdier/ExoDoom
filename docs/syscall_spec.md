...
| 5  | `exo_get_ticks()`                   | Timer       | ✅     | Return `uint32_t` milliseconds since boot. Zero arguments. Used by `DG_GetTicksMs` and `DG_SleepMs`. **Implemented (SCRUM-33)** — `sys_get_ticks` in `src/sys_time.c`, bound by `sys_time_init()`. The first syscall wired end to end; `tests/kernel/test_sys_time_k.c` calls it from ring 3 and checks it against `kernel_get_ticks_ms()`. Widened from `uint32_t` so it is never negative and never mistaken for an error code. |
...
bind with `exo_syscall_register()`. SCRUM-32 delivered the entry path and bound
nothing; SCRUM-33 bound the first one (`exo_get_ticks`, #5, via
`sys_time_init()` in `src/sys_time.c`). Each subsystem registers its own numbers
from `kernel_main`, ahead of the `TESTING` branch so the test suite exercises
the same table a normal boot has. Everything still unbound answers
`-EXO_ENOSYS`.
