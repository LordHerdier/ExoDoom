...
| SCRUM-143 Storage & File I/O | To Do (epic), ATA driver now done | ATA PIO driver ✅ SCRUM-102 (`src/ata.c/h`, polled, primary bus/master, no syscall surface); still open: `exo_disk_read`/`exo_disk_write` syscalls ⬜ SCRUM-103 (blocked on nothing now), disk ownership/binding table ⬜ SCRUM-188, ported FAT-like fs ⬜ SCRUM-189, FAT-formatted QEMU disk image ⬜ SCRUM-190, `exo_file_*` syscalls, ramdisk save persistence (SCRUM-42/44/75/92/93/104/106) |
