...
calls `speaker_tick()` each tick to end a timed tone, so the
`exo_sound_tone` syscall (SCRUM-100, `src/syscall_sound.c`) is non-blocking.
...
| Sound       | `exo_sound_tone`, `exo_sound_stop` ✅ SCRUM-100                     | Sprint 11  |
...
| SCRUM-146 Audio | In Progress | PC speaker driver ✅ SCRUM-98 (`src/speaker.c/h`, PIT channel 2, non-blocking duration via IRQ0); SFX→tone table ✅ SCRUM-99 (`src/doom_sfx_tone.c/h`, 1–4 freq/duration steps per `sfxenum_t` id); `exo_sound_tone`/`exo_sound_stop` syscalls ✅ SCRUM-100 (#17/#18, `src/syscall_sound.c/h`; ring-3 port access to the speaker proven to #GP); Doom `sound_module_t` ✅ SCRUM-101 (`src/doom_sound.c/h`, `FEATURE_SOUND` on, one-voice sequencer driven from `I_UpdateSound`, Doom priority rule; music is a silent module). All four stories done — `docs/syscall_spec.md` §6 Option B. **Real PCM audio has since started on top of that:** PCI enumeration + config space ✅ SCRUM-209 (`src/pci.c/h`, mechanism #1 over 0xCF8/0xCFC, `docs/drivers/pci.md`) and the Intel HDA controller driver ✅ SCRUM-210 (`src/hda.c/h`, CORB/RIRB codec ring + BDL stream DMA + INTx completion interrupt, `docs/drivers/hda.md`) — a synthesized tone now plays by DMA out of system memory, with register/DMA/IRQ state asserted by the `hda` KUnit suite. The other end of that path is now real too: the WAD DMX sound lump decoder ✅ SCRUM-211 (`src/doom_dmx.c/h`, `DS*` → 8-bit unsigned PCM, zero-copy out of the mounted module, 16 padding samples stripped at each end), acceptance-tested against the vendored `freedoom2.wad` by the `doom_dmx` suite. **That decode disproved the ticket's premise and the mixer must not inherit it:** DMX lumps are not "almost universally 11025 Hz" — of freedoom2 v0.13.0's 109 `DS*` lumps, 67 are 22050 Hz, 38 are 11025, 2 are 17990, 1 is 16000 and 1 is 44100, so with HDA pinned at 48 kHz/16-bit/stereo, resampling is unavoidable rather than optional. **And the two ends are now joined:** the software PCM mixer ✅ SCRUM-212 (`src/pcm_mixer.c/h`, 8 voices matching Doom's `snd_channels`, per-voice 16.16 phase accumulator with linear interpolation so each lump plays at *its* rate, int32 summation saturating to int16, Doom's own squared-separation panning law) plus a streaming mode in the driver (`hda_pcm_start()`, the BDL rediced from 2 entries to `HDA_BDL_ENTRIES_PCM` = 16 so a slice is ~21 ms rather than ~171 ms, refilled from the stream-completion IRQ) — `docs/drivers/hda.md` §12. Overlap and the non-disturbance of frame timing are both asserted from hardware state by the `hda_pcm` suite, the latter as a ratio between the PIT clock and the audio DMA rate, since timing a wait against the clock that gates it proves nothing; the arithmetic itself is asserted sample-for-sample by the hardware-free `pcm_mixer` suite. **And ring 3 can now reach it:** the PCM sound syscalls ✅ SCRUM-213 (`exo_sound_pcm` #27 / `exo_sound_pcm_stop` #28, `src/syscall_sound.c`), which resolved the story's own open design question by making sound **shared rather than exclusively bound** — there is no `exo_sound_acquire` and no `src/sound_binding.c`, because `pcm_mixer_start()`'s eight voices, Doom priority rule and non-disturbing refusal already are the admission policy a binding table would answer worse; what is owned is a voice, not the device (`docs/syscall_spec.md` §3.5b). The samples are copied into kernel pages rather than played in place: the mixer renders from `hda_irq_handler()` under whatever CR3 is loaded, and the caller may free its buffer the moment the syscall returns. A ring-3 LibOS queueing its own samples and the controller then DMA'ing them is asserted end to end by the `syscall_sound_pcm` suite. The ring-0 boot demo SCRUM-212 added to `kernel_main` is gone with it — the boot tone stays. Still open on that line: Doom's sound module is still SCRUM-101's one-voice speaker sequencer, and `src/revoke.c` has no sound leg (an exiting LibOS loses its voices, a revoked one does not) |
