...
**SCRUM-89 (game loop timing verification): ✅ confirmed, under KVM.** The
PIT-derived clock does produce a real 35 tics/sec of simulated game time —
`src/libos_doom/libos_doom.c`'s `doom_tic_rate_sample()` samples Doom's own
`gametic` global (not draw-call count — see that function's comment for why
render calls are a poor proxy) every 10 sim-seconds and reports it to serial.
Five consecutive windows at the default 1024x768 framebuffer measured
34.96/31.39/35.00/35.10/34.90 tics/sec — four within ~0.3% of the 35.00
reference, one transient dip plausibly attributable to a state-transition or
host-scheduling blip rather than a systemic pacing defect. Two things
observed along the way, **out of this ticket's scope**, are worth flagging
for whoever picks them up: (1) `DG_DrawFrame`'s blit cost is
resolution-dependent and, at a large enough framebuffer (worst at QEMU
fullscreen), can outrun single-player Doom's 5-tic ticcmd build-ahead cap
(`BuildNewTic`, `src/doom/d_loop.c`) and genuinely slow the game down, not
just drop rendered frames — this is SCRUM-78's ("optimize `DG_DrawFrame`...
<5ms measured via serial profiling") to fix, not a regression introduced
here; (2) under TCG (no `-enable-kvm`), the measured rate drops to roughly
60% of target — a QEMU virtual-clock/PIT-emulation artifact of that mode,
not a kernel or Doom defect, and exactly why this verification requires KVM
to mean anything.
...
| **Sprint 9: Playability E1M1** _(13 Jul – 27 Jul)_      | Playable first level                     | Debug and fix E1M1 rendering (walls, floors, ceilings, sprites) ✅ SCRUM-84, verify combat (shooting, enemy AI, damage, pickups, status bar), menu navigation (new game, options, difficulty, quit), performance profiling (frame time per subsystem), fix top 3 bottlenecks, verify 35 tics/sec game loop timing ✅ SCRUM-89 (§5.3), test with Freedoom2 IWAD and original DOOM2.WAD                                                                                      |
...
| SCRUM-144/145 Doom Port & Gameplay Verification | In Progress (epic) | Doom IS linked into `build/exodoom` and playable (SCRUM-66/77/79) — this row was stale; §5.3 has the up-to-date rundown of what's landed vs. open under Sprint 8/9's original scope |
...
| SCRUM-143 Storage & File I/O | To Do | `exo_file_*` syscalls, ATA PIO driver, ramdisk save persistence (SCRUM-42/44/75/102–106) — untouched |
