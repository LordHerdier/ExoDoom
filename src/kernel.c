...
static void mmap_type_color(fb_console_t* con, uint32_t type) {
    switch (type) {
        case MULTIBOOT_MMAP_AVAILABLE:
            fbcon_set_color(con,  80, 210,  80,  0, 0, 0); break;  // green
        case MULTIBOOT_MMAP_ACPI_RECLAIM:
        case MULTIBOOT_MMAP_ACPI_NVS:
            fbcon_set_color(con, 220, 190,  60,  0, 0, 0); break;  // yellow
        case MULTIBOOT_MMAP_BADRAM:
            fbcon_set_color(con, 230,  50,  50,  0, 0, 0); break;  // red
        default:
            fbcon_set_color(con, 140, 140, 140,  0, 0, 0); break;  // gray
    }
}
...
void kernel_main(uint32_t mb_info_addr) {
    ...
    if (!(mb->flags & MULTIBOOT_INFO_FLAG_MODS) || mb->mods_count == 0) {
        klog(&con, kernel_get_ticks_ms(), "No multiboot modules — skipping WAD display.");
        qemu_exit(0);
    }
    ...
    // Clear to black and blit flat grid (scale=1: 64x64 per flat, 16x12 = 192 cells)
#define FLAT_SCALE 1
