...
/*
 * libos_heap.c -- first-fit, segmented free-list allocator backed by
 * libos_page_alloc() (SCRUM-38). See libos_heap.h for the design writeup;
 * this is src/heap.c's algorithm with the page source swapped and the
 * segment-growth contiguity check done on virtual, not physical, addresses.
 *
 * A "segment" is one run of virtually contiguous pages obtained from
 * successive libos_page_alloc() calls. That allocator hands back adjacent
 * vaddrs as long as nothing has freed a slot out from under it -- true in
 * practice for a heap that only ever grows, but not guaranteed, so growth
 * checks contiguity explicitly rather than assuming it: a non-adjacent page
 * starts a new segment instead of corrupting the block list of an existing
 * one. First-fit search walks every block in every segment; a block's size
 * never spans a segment boundary.
 *
...
typedef struct block {
    ...
    int free;
    struct block *prev;   /* virtually previous block, same segment, or NULL */
    struct block *next;   /* virtually next block, same segment, or NULL */
} block_t;
...
/* One past the virtual address of the most recently libos_page_alloc()'d
 * page, or 0 before the first call -- lets libos_heap_grow_one_page() tell
 * whether the next page continues the current segment or must start a new
 * one. */
...
/* Extend the current segment by exactly one virtually-contiguous page. */
static void segment_grow(segment_t *seg, uintptr_t vaddr) {
    ...
    if (tail->free) {
        ...
        segment_append_block(seg, tail, vaddr);
    }
}

/* Start a brand new segment out of one page. */
static void segment_create(uintptr_t vaddr) {
    segment_t *seg = (segment_t *)vaddr;
    seg->start = vaddr;
    seg->end   = vaddr + PAGE_SIZE;
    seg->next  = NULL;

    block_t *first = (block_t *)(vaddr + sizeof(segment_t));
...
/* Grow the heap by exactly one page from libos_page_alloc(). Returns 0 on
 * success, -1 if the underlying allocator is out of pages. */
static int heap_grow_one_page(void) {
    void *page = libos_page_alloc();
    if (page == NULL) {
        serial_print("libos_heap: out of pages\n");
        return -1;
    }

    uintptr_t vaddr = (uintptr_t)page;

    if (segments_tail != NULL && vaddr == heap_growth_cursor) {
        segment_grow(segments_tail, vaddr);
    } else {
        segment_create(vaddr);
    }

    heap_growth_cursor = vaddr + PAGE_SIZE;
    return 0;
}
...
void libos_heap_free(void *ptr) {
    ...
    if (b->free) {
        serial_print("libos_heap: double free detected\n");
        return;
    }
...
void *libos_heap_realloc(void *ptr, size_t size) {
    ...
    void *newp = libos_heap_alloc(size);
    if (newp == NULL) {
        return NULL;
    }
...
/* ---- Accounting ---------------------------------------------------------
 *
 * Walks every block of every segment; see libos_heap.h for what the
 * counters mean and why used_bytes rather than free_bytes is the one to
 * compare when the heap may have grown in between.
 */
...
void libos_heap_report(const char *label) {
    libos_heap_stats_t s;
    libos_heap_get_stats(&s);

    /* printf() has no length modifiers (see src/stdio.h), so %u is an
     * unsigned int -- narrow the size_t counters deliberately, same as
     * heap_report() in src/heap.c. */
    printf("libos_heap[%s]: free=%u used=%u total=%u "
